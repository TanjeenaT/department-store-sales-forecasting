...
plot(retail_data,cex.main = 0.8, type='o',ylab="Department stores retail", main="Figure 1.1: Time Series plot of total number of department stores in retail ")
plot(retail_data,cex.main = 0.8,,type='l', ylab = "Department stores retail", main="Figure 1.2: Time series plot of total number of department stores in retail.")
...
acf(as.vector(retail_data), main = "Figure 1.3: ACF of number of department stores in retail", xlab = "Lag", ylab = "ACF", lag.max=50)
pacf(as.vector(retail_data), main = "Figure 1.4: PACF of total number of department stores in retail", xlab = "Lag", ylab = "PACF",ylim = c(-1, 1))

# first lag scatter plot
lag1_retail_data <- zlag(retail_data)
index = 2:length(retail_data)
plot(y=retail_data,x=lag1_retail_data, ylab='Number of stores', xlab='First lag', main= "Figure 1.5: Scatter plot of neighboring number of stores",cex.main = 0.8)
...
plot(y=retail_data,x=lag2_retail_data, ylab='Number of stores', xlab='Second lag', main= "Figure 1.6: Scatter plot of number of stores and its second lag values",cex.main = 0.8)
...
check_normality(retail_data, "Figure 2.1: QQ plot of number of department stores in retail")
# Transformation 
...
plot(rt_dataBC, type = "o", ylab = "Box-cox transformed data", xlab = "Time (Months)", main = "Figure 2.2: Box-Cox transformed data of number of department stores in retail", cex.lab = 0.5, cex.main = 0.8)
...
plot(rt_dataBCDiff, type = "o", ylab = "Number of department stores", xlab = "Time (Months)", main = "Figure 2.4: First Difference of Transformed data of number of department stores.", cex.main = 0.8)

# ACF & PACF plot of 1st difference on transformed series
acf(rt_dataBCDiff, main = "Figure 2.5: ACF of first difference of transformed data", xlab = "Lag", ylab = "ACF", lag.max = 80, cex.main = 0.8)
pacf(rt_dataBCDiff, main = "Figure 2.6: PACF of first difference of transformed data", xlab = "Lag", ylab = "PACF",ylim = c(-1, 1), cex.main = 0.8)
...
plot(rt_dataBCDiffSD, type = "o", ylab = "Number of department stores", xlab = "Time (Months)", main = "Figure 2.7: First & seasonal difference of transformed data.", cex.main = 0.8)
...
acf(rt_dataBCDiffSD, main = "Figure 3.1: ACF of first & seasonal difference of transformed data", xlab = "Lag", ylab = "ACF", lag.max = 80, cex.main = 0.8)
pacf(rt_dataBCDiffSD, main = "Figure 3.2: PACF of first & seasonal difference of transformed data", xlab = "Lag", ylab = "PACF",ylim = c(-1, 1), cex.main = 0.8)
...
###### MODEL_FITTING USING ML, CSS_ML, CSS(CSS some models failed and were assigned NA)
library(forecast)
library(lmtest)
...
orders_to_fit <- list(
  ...
  c(2, 0, 3), c(5, 0, 3), c(5, 0, 4), c(6, 0, 3), c(6, 0, 4),
  c(7, 0, 4)
)
...
#fit_css_ml_5_1_4 <- all_results[["CSS-ML"]][["SARIMA(5,0,4)x(1,1,2)[12]_Method-CSS-ML"]]
#fit_ml_5_1_3 <- all_results[["ML"]][["SARIMA(5,0,3)x(1,1,2)[12]_Method-ML"]]
...
par(mfrow = c(1, 2))
# Histogram
hist(res.513,xlab='Standardized Residuals',main="Fig 5.4 (a) Residuals from the SARIMA(5,1,3)x(1,1,2)_12 Model")

#QQ plot
qqnorm(res.513,main="Fig5.4 (b): Q-Q plot for Residuals: SARIMA(5,1,3)x(1,1,2)_12 Model.")
qqline(res.513)
...
par(mfrow = c(1, 2))
# Histogram
hist(res.613,xlab='Standardized Residuals',main="Fig 5.7 (a) Residuals from the SARIMA(6,1,3)x(1,1,2)_12 Model")

#QQ plot
qqnorm(res.613,main="Fig 5.7 (b): Q-Q plot for Residuals: SARIMA(6,1,3)x(1,1,2)_12 Model.")
qqline(res.613)
...
#-----------CSS213---------------------
fit_css_ml_2_1_3 <- Arima(rt_dataBC, order = c(2,1,3), seasonal = c(1,1,2), method = "CSS-ML")

res.213 <- rstandard(fit_css_ml_2_1_3)

par(mfrow = c(1, 2))
# Histogram
hist(res.213,xlab='Standardized Residuals',main="Fig 5.1 (a) Residuals from the SARIMA(2,1,3)x(1,1,2)_12 Model")

#QQ plot
qqnorm(res.213,main="Fig 5.1 (b): Q-Q plot for Residuals: SARIMA(2,1,3)x(1,1,2)_12 Model.")
qqline(res.213)
